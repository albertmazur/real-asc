    ...
    <div class="shadow p-3 mb-5 bg-body-tertiary rounded">
        <form action="{{ route("comment.index") }}" method="get">
            <div class="mb-3">
              ...
              <div class="mb-3">
                <label for="sortWhoSearch" class="form-label">{{ __("Użytkownik") }}</label>
                <select name="sortWhoSearch" class="form-select" aria-label="{{ __("Wybierz użytkownika") }}">
                    <option @if ($sortWhoSearch== -2) selected @endif value="-2">{{ __("Wszyscy") }}</option>
